...
public class EditForm extends JFrame {
	...
	private JButton getDone() {
		if (done == null) {
				...
				Form f=new Form();
				f.setId(cid);
				f.setFname(fname.getText());
				f.setMname(mname.getText());
				f.setLname(lname.getText());
				 f.setT_address(taddress.getText());  
				 f.setP_address(paddress.getText());
				 f.setUsername(username.getText());
				 f.setPassword(password.getText());	
				f.setEmail(email.getText());	
				f.setDob(new Date(dob.getDate().getTime()));
				f.setPhone(phone.getText());
			...
			done.setFont(new Font("Dialog", Font.PLAIN, 15));
			done.setBackground(new Color(30, 144, 255));
			done.setForeground(Color.WHITE);
			done.setBounds(272, 424, 117, 25);
		}
		return done;
	}
	...
	private JLabel getLabel_1() {
	  ...
	  public void setData(int id) {
		  ...
		  Service s=new ServiceImpl();
		  Form f=s.getById(id);
		   fname.setText(f.getFname());
		   mname.setText(f.getMname());
		   lname.setText(f.getLname());
		   email.setText(f.getEmail());
		   phone.setText(f.getPhone());
		   dob.setDate(f.getDob());
		   paddress.setText(f.getP_address());
		   taddress.setText(f.getT_address());
		   username.setText(f.getUsername());
		   password.setText(f.getPassword());
		   if(f.getGender().equalsIgnoreCase("male")) {
			   male.setSelected(true);
		   }
		   else if(f.getGender().equalsIgnoreCase("Female")) {
